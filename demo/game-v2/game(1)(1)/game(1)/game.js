...
(() => {
  ...
  function loadMapConfig(mapConfig) {
    ...
    // Load NPCs and convert waypoints to pixel coordinates.
    ...
    // Build runtime door and chest state.
  ...
  function updatePlayer(dt) {
    ...
    // Check whether the player actually moved.
    ...
    if (moved) {
      // Sprinting always leaves footprints.
      if (sprinting) {
        addFootstepsAlongPath(oldCenterX, oldCenterY, newCenterX, newCenterY, ix, iy);
      } else {
        // Walking only leaves footprints when an NPC can see the player.
  ...
  function updateFootsteps(dt) {
    const now = Date.now();
    const lifetime = mapSettings.footstepLifetime || 3000;
    
    // Fade older footprints first.
    state.footsteps.sort((a, b) => a.timestamp - b.timestamp);
    
    // Drop expired footprints.
  ...
  function updateInteractionTargets() {
    ...
    if (state.nearestChest) {
      state.prompt = 'Press E to open the chest';
    } else if (state.nearestDoor) {
      state.prompt = state.nearestDoor.open ? 'Press E to close the door' : 'Press E to open the door';
    }
  }
...
function lineOfSight(x0, y0, x1, y1) {
  ...
  // Use Bresenham stepping for line-of-sight checks.
  ...
  while (!(x === tx1 && y === ty1)) {
      ...
      // Do not test the destination tile itself as an obstacle.
      if (x === tx1 && y === ty1) break;

      // Stop if the current tile blocks vision.
      if (isVisionBlocker(x, y)) return false;
  }
  return true;
}

// Check whether a tile blocks vision.
function isVisionBlocker(tx, ty) {
  ...
  // Closed doors also block vision.
  for (const door of doors) {
    if (door.tiles) {
...
// Build a continuous visible area for cone rendering.
function getVisibleArea(npc) {
  const nc = npcCenter(npc);
  const range = mapSettings.visionRange || 112;
  const stepSize = 2;
  
  // Pick the angle span from the NPC facing direction.
  ...
  // Accumulate sampled boundary points.
  const pathPoints = [];
  
  // Sweep the cone and find the furthest visible point for each angle.
  const angleStep = 0.01;
  for (let angle = startAngle; angle <= endAngle; angle += angleStep) {
    let maxDist = 0;
    
    // Probe outward along the current angle.
...
// Sample visible pixels for a denser visibility test.
function getVisiblePixels(npc) {
  ...
  // Sample the cone at a higher density.
  ...
  for (let angle = startAngle; angle <= endAngle; angle += angleStep) {
    for (let dist = stepSize; dist <= range; dist += stepSize) {
      ...
      // Keep pixels that are still visible.
      if (hasLineOfSightToPixel(nc.x, nc.y, px, py)) {
...
// Check line of sight to a specific pixel.
function hasLineOfSightToPixel(x0, y0, x1, y1) {
  // Use a finer Bresenham-style step.
  ...
  while (!(Math.abs(x - x1) < 2 && Math.abs(y - y1) < 2)) {
    ...
    // Stop if the current sample falls on a blocking tile.
...
// Build a list of visible tiles for fallback logic.
function getVisibleTiles(npc) {
  const nc = npcCenter(npc);
  const npcTileX = Math.floor(nc.x / baseTile);
  const npcTileY = Math.floor(nc.y / baseTile);
  const range = Math.floor((mapSettings.visionRange || 112) / baseTile);
  
  const visibleTiles = [];
  
  // Collect candidate tiles from the facing direction.
  const candidates = getVisionCandidates(npcTileX, npcTileY, range, npc.facing);
  
  // Keep only tiles with a clear line of sight.
...
// Generate candidate tiles for a quarter-cone field of view.
function getVisionCandidates(x, y, range, facing) {
  const candidates = [];
  
  for (let i = 1; i <= range; i++) {
    const half = Math.min(2, Math.floor(i / 2));
...
// Check line of sight to a specific tile.
...
// Map the alert meter to a simple alert level.
...
function updateNPC(npc, dt) {
  ...
  // Alert meter system.
  if (sees) {
    // Seeing the player raises alert quickly.
    npc.alert = Math.min(100, npc.alert + 8);
    npc.loseSight = 0;
    
    // Refresh the last seen player position.
    const pc = playerCenter();
    npc.lastSeenX = pc.x;
    npc.lastSeenY = pc.y;
  } else {
    // Losing sight slowly reduces alert.
    npc.alert = Math.max(0, npc.alert - 0.5);
    npc.loseSight += 1;
  }

  // State machine with simple priority rules.
  if (sees && npc.state !== 'CHASE') {
    // Switch to chase as soon as the player is seen.
    npc.state = 'CHASE';
  } else if (!sees && npc.state === 'CHASE') {
    // Lose sight during chase and fall back to search.
    npc.state = 'SEARCH';
    npc.searchTimer = mapSettings.searchDuration || 3;
    npc.searchWanderTimer = 0;
    
    // Search around the player's last known position.
    npc.searchTargetX = npc.lastSeenX;
    npc.searchTargetY = npc.lastSeenY;
    
    // Create a visible search marker.
    npc.searchMarker = {
      x: npc.lastSeenX,
      y: npc.lastSeenY,
      timestamp: Date.now()
    ...
    // Pick a new random search point every 0.5 seconds.
    if (npc.searchWanderTimer >= 0.5) {
      npc.searchWanderTimer = 0;
      
      // Sample a random point around the search center.
      const wanderRadius = mapSettings.searchRadius || 48;
    ...
    // Move toward the current random search point.
    seek(npc, npc.searchTargetX, npc.searchTargetY, npc.speedPatrol, dt);
    
    // Return to patrol when the search timer expires.
    if (npc.searchTimer <= 0) {
      npc.state = 'PATROL';
      npc.searchMarker = null;
    }
  } else if (npc.state === 'PATROL') {
    // Clear leftover search state while patrolling.
    npc.searchMarker = null;
  }
...
function drawNPCs(ctx, scale) {
  ctx.save();
  for (const npc of npcs) {
    // Draw a continuous red vision cone.
    ...
    if (visibleArea.length > 0) {
      ...
      // Start at the NPC center.
      ctx.moveTo(nc.x * scale, nc.y * scale);
      
      // Trace the vision boundary.
      for (const point of visibleArea) {
        ctx.lineTo(point.x * scale, point.y * scale);
      }
      
      // Close the path back to the NPC.
      ctx.closePath();
      ctx.fill();
    }
...
function drawUI() {
  ...
  if (!info) return;
  let promptLine = state.prompt ? `<p>${state.prompt}</p>` : '<p>Move near a door or chest and press E to interact.</p>';
  ...
  info.innerHTML = `
    ...
    <p>Move with WASD or the arrow keys, Shift to sprint, E to interact, and ~ to toggle collision.</p>
...
async function initGame(level = 1) {
  try {
    // Switch levels through the map manager when available.
    if (typeof mapManager !== 'undefined') {
      ...
      throw new Error(`Level ${level} config file was not found`);
    }

    // Runtime map name lookup.
    const mapNames = {
      1: 'basic test',
...
// Expose the runtime entry point to the page.
