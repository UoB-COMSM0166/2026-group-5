...
export async function preloadLoseScreen(p) {
  const capturedUrl = new URL("../assets/images/original/drawings/captured.png", import.meta.url).href;
  captured = await p.loadImage(capturedUrl);
}
export function drawLoseScreen(p) {
  const now = p.millis();
  const t = now * 0.02;
...
function drawCaptured(p, t, layout) {
