...
export function createBoxSystem(mapData, existingChests = [], tileSize = 16) {
  const boxes = [];
  ...
  if (boxes.length) return api(boxes);
  if (!mapData?.layers) return api(boxes);
  const layer = mapData.layers.find((entry) => String(entry.name || '').toLowerCase() === 'box' && entry.type === 'tilelayer');
  if (!layer || !Array.isArray(layer.data)) return api(boxes);
