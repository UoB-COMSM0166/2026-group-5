...
export function drawIntroScreen(p, state) {
  if ((state.screenTimeMs || 0) < 50) skipRequested = false;
  ...
  for (const item of introLines) {
    ...
    y += lineSpacing;
    if (item.gapAfter) y += sy(item.gapAfter, layout);
  }

  if (isAllVisible(elapsed)) {
  ...
  state.prompt = isAllVisible(elapsed) ? 'Press Enter to continue' : 'Press Enter to skip';
}
...
function isAllVisible(elapsed) {
  const last = introLines[introLines.length - 1];
  return !!last && isFullyVisible(last.text, elapsed, last.delay);
}

export function handleIntroScreenKey(key, state, api) {
  if (key !== 'Enter') return false;
  const elapsed = state.screenTimeMs || 0;
  const allVisible = isAllVisible(elapsed);
  if (!allVisible && !skipRequested) {
    skipRequested = true;
    api?.setMessage?.('Skipping intro…', 0.6);
    return true;
  }
  skipRequested = false;
