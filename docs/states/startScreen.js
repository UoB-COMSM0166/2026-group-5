...
export function drawStartScreen(p, state) {
...
function drawDragon(p, dragon, t, layout) {
...
function drawTitle(p, titleImg, t, layout) {
...
function drawMenuButtons(p, layout) {
  ...
  for (let i = 0; i < menu.options.length; i++) {
    ...
    drawButton(p, startX, y, w, h, menu.options[i], i === menu.selectedIndex, layout);
  }
}
...
export function handleStartScreenKey(key, state, api) {
  if (key === 'ArrowUp') {
    menu.selectedIndex = (menu.selectedIndex - 1 + menu.options.length) % menu.options.length;
    api?.setMessage?.(menu.options[menu.selectedIndex], 0.6);
    return true;
  }
  if (key === 'ArrowDown') {
